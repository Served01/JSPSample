		...
		function checkId() {	
			...
			if (userId.length < 4 || userId.length > 15) {  // userId 필드 내용의 길이가 4 이하이거나 15 이상일 경우 실행
		...
		function checkPw() {			
			var pw1 = document.getElementById('user-pw1').value;
			var pw2 = document.getElementById('user-pw2').value;
			
			if (pw1.length < 8 || pw1.length > 15) {
        		alert("비밀번호는 8자리 이상 15자리 이하 이어야 합니다.");  // 오류 메시지 표시
